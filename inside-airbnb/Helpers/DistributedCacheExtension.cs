...
namespace inside_airbnb.Helpers
{
    ...
    {
        public static async Task<string> GetDataFromCacheAsync(this IDistributedCache distributedCache, string cacheKey)
        {
            return await distributedCache.GetStringAsync(cacheKey);
        }

        public static async Task AddDataToCacheAsync<T>(this IDistributedCache distributedCache, string cacheKey, T data, int expiresInSeconds)
        {
